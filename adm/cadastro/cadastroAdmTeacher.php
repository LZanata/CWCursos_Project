...
if (isset($_POST['submit'])) {
    ...
    $email = $_POST['email'];
    $senha = password_hash($_POST['senha'], PASSWORD_DEFAULT); // Utiliza hash para a senha
    ...
    if ($_POST['senha'] !== $confisenha) {
        echo "<script>alert('As senhas não coincidem!');</script>";
        exit;
    }

    // Verifica se o usuário ou e-mail já estão cadastrados na tabela `usuarios`
    ...
    $insertQuery = "INSERT INTO usuarios (nome, usuario, email, senha, data_nascimento, tipo, status) VALUES (?, ?, ?, ?, ?, ?, 'ativo')";
    $stmt = $conn->prepare($insertQuery);
    $stmt->bind_param("ssssss", $nome, $usuario, $email, $senha, $dataNascimento, $tipoUsuario);
